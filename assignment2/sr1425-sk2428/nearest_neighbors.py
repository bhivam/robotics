...
def get_k_nearest_neighbors_arm(k, configs, origin):
    ...
    for config in configs:
        ...
        distance = np.linalg.norm(np.array(xy_config_coordinates) - np.array(xy_coordinates))
        distances.append((distance, config))
    distances.sort(key=lambda x: x[0])
    return get_configurations_from_tuple(distances)[:k]
...
def visualize_robot_neighbors(target, configs, name="visualize.png"):
    ...
    robot_target = plt.Rectangle((length/2, width/2), length, width, fill=True, color='r')
...
if __name__ == "__main__":
    ...
    if (args.robot == 'arm'):
        ...
        nearest_neighbors = get_k_nearest_neighbors_freebody(args.k, configs, args.target)
